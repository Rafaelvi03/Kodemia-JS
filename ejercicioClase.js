    ...
    function convertirGrados(centigrados){
...
/*generar una funcion que me ayude a eliminar un elemento del
arreglo, el arreglo y el elemento a eliminar seran los parametros de esta funcion 
...
function eliminarArray(remover = 2){
...
eliminarArray(); // Status: incompleto 

/*crear una funcion que retorne la palabra(parametro1) repetida el numero d
e veces que se indique(parametro2) repeatString('hey', 3) // returns 'heyheyhey'
*/


function retornarPalabras (cadenaRepetida, repetir){
  repetir = 'hey';
  let StringRepetidos= cadenaRepetida=repetir.repeat(3)
  return StringRepetidos;

  
}

console.log(retornarPalabras('hey'))


/*
Generar una funcion que retorne la palabra(parametro)
 al reves reverseString('hola') // returns 'aloh'*/

function palabraAlreves(stringAlreves){
    let cadenaInvertida = '';
    for(i= stringAlreves.length -1; i>=0; i--){
        cadenaInvertida += stringAlreves[i]
    }
    return cadenaInvertida
}
console.log(palabraAlreves('hola'))

    /*Generar una funcion que nos ayude a retornar la suma de todos los 
    numeros que hay dentro de los parametros sumAll(1, 4) // 1 + 2 + 3 + 4 = 10
*/
function suma(sumaNumero, sumaNumeroSiguiente) {
    }
}
...
const users = [
  ...
  const age = users.find(function(edad){
   let  resultado = edad.dob.date
